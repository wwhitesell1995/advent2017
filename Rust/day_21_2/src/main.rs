...
fn main() {
    let filename = "day21_2_input.txt";
    ...
    let num_pixels_on = get_num_pixels_on(&starting_pattern, &rulebook, 18);
    println!(
        "The number of pixels that are on after 18 iterations is: {0}",
        num_pixels_on
    );
}
