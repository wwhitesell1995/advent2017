defmodule TwistyMazeDecrement do
  ...
  def offset(numlist,jump_no, get_num) when get_num>=3 do
    List.update_at(numlist,jump_no,fn(x)->x-1 end)
  end
  ...
  def jump_around(numlist, jump_no, incrementor, list_size) when jump_no>=0 and jump_no<list_size do
      ...
      new_numlist=offset(numlist,jump_no,get_num)
